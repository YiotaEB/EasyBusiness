...
public class AddCustomersFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        v = inflater.inflate(R.layout.fragment_add_customers, container, false);
        ...
        customerButton = v.findViewById(R.id.add_customer_Btn);
        nameEditText = v.findViewById(R.id.customer_name_edittext);
        addressEditText = v.findViewById(R.id.customer_address_edittext);
        telephoneEditText = v.findViewById(R.id.customer_telephone_edittext);
        cityEditText = v.findViewById(R.id.customer_city_edittext);
        countrySpinner = v.findViewById(R.id.customer_country_spinner);
        //productsButton = v.findViewById(R.id.add_customer_select_products_Btn);
            ...
            try {
                ...
                if (statusCode == HttpURLConnection.HTTP_OK) {
                    ...
                    activity.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Spinner countriesSpinner = view.findViewById(R.id.customer_country_spinner);
    ...
    private class AddCustomerAsyncTask extends AsyncTask<Void,Void,Void> {
        ...
        public AddCustomerAsyncTask(Activity activity, View view, String sessionID,  String name, String city, String address, String telephone, int countryID) {
        ...
        protected Void doInBackground(Void... params) {
            ...
            try {
                URL url = new URL(AsyncTasks.encodeForAPI(activity.getString(R.string.baseURL), "Customers", "Create"));
                HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
                urlConnection.setDoOutput(true);
                byte[] outputBytes = query.getBytes("UTF-8");
                urlConnection.setRequestMethod("POST");
                ...
                if (statusCode == HttpURLConnection.HTTP_OK) {
                    ...
                    if (status.equals(AsyncTasks.RESPONSE_OK)) {
                        activity.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(activity, "Customers '" + name + "' created.", Toast.LENGTH_LONG).show();
